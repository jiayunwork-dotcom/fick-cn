...
// TestBoundaryParseKinds checks that both supported condition names parse
// and that anything else is rejected.
...
// TestBoundaryFluxClosedIsZero verifies that a no-flux end contributes
// exactly zero mass flux, however large the field gradients are.
...
// TestBoundaryDirichletFluxNonzero verifies a Dirichlet end does carry a
// flux, and that its sign matches the gradient direction: high boundary
// value pushes mass in (positive flux).
func TestBoundaryDirichletFluxNonzero(t *testing.T) {
	...
	// theta-averaged gradient (c0 - c1) is positive, so the flux is positive.
...
// TestBoundaryPairClassification checks the rod classification helpers.
...
// TestLeftRightRowsDirichlet verifies the Dirichlet boundary rows are the
// identity so the pinned value is preserved exactly.
